...
void setup(void) {
  ...
  // Set device name 
  ble.println("AT+GAPDEVNAME=ADV Rally");
  ble.println("ATZ");
...
void loop(void){
  if ( ble.isConnected() ) {
    if ( digitalRead(9) == LOW && digitalRead(10) == LOW ) {
      ...
      ble.println("MEDIASTOP");
      Serial.println("MEDIASTOP");
    } else if ( digitalRead(inputPins[0]) == LOW ) {
      ble.print("AT+BleHidControlKey=");
      ble.println("VOLUME+");
      Serial.println("VOLUME+");
    } else if ( digitalRead(inputPins[1]) == LOW ) {
      ble.print("AT+BleHidControlKey=");
      ble.println("VOLUME-");
      Serial.println("VOLUME-");
    } else if ( digitalRead(inputPins[2]) == LOW ) {
      ble.print("AT+BleHidControlKey=");
      ble.println("MEDIANEXT");
      Serial.println("MEDIANEXT");
    } else if ( digitalRead(inputPins[3]) == LOW ) {
      ble.print("AT+BleHidControlKey=");
      ble.println("MEDIAPREVIOUS");
      Serial.println("MEDIAPREVIOUS");
    } else if ( digitalRead(inputPins[4]) == LOW ) {
      ble.print("AT+BleHidControlKey=");
      ble.println("VOLUME+");
      Serial.println("VOLUME+");
    } else if ( digitalRead(inputPins[4]) == LOW ) {
